      ...
      <div className='grid gap-5 w-full pt-4'>
          {
            ...
            fetchTaskPerformers?.map((detail, index) => (
              <div key={index} className='w-full px-6 pt-6 pb-[31px] bg-gray-300 dark:bg-zinc-800 rounded-lg flex flex-col gap-y-4 justify-between items-start gap-2'>                
                <div className='flex items-start w-full justify-between'>
                  <div className='flex items-start gap-x-4'>
                    <img
                        className='w-[47px] h-[47px]  rounded-[14.95px] border-2 border-fuchsia-600'
                        src={detail?.user?.profile_picture}
                      />
                      <div className='flex flex-col gap-y-4'>
                        <div>
                          <span className='text-[10px] text-[#B1B1B1]'>@{detail?.user?.username}</span>
                          <p className='text-[14px] text-[#FFFFFF]'>{detail?.user?.email}</p>
                          <span className='text-[10px] text-[#909090]'>{detail?.date_completed &&
                          format(
                        ...
                        <div className='flex flex-col gap-y-6'>
                          <div className='flex items-center gap-x-2'>
                              ...
                              <span className='text-[12px] text-[#B1B1B1]'>{detail?.account_name}</span>
                          </div>
                          <div className='flex flex-col gap-y-4'>
                              <p className='text-[#D8D8D8] text-[12px]'>Select one of the options below after verifying the task</p>
                    ...
                    <div>
                      <p className='text-[#D8D8D8] text-[12px]'>Screenshot / Proof Of Work</p>
                      {
                        detail?.proof_screenshot_path && (
                          <span className='text-[#FF6DFB] text-[12px]'  onClick={() =>
                      ...
                      {
                        detail?.post_link && (
                          <a href={detail?.post_link} className='text-[#FF6DFB] text-[12px]'>Click here to view</a>
                        )
                      }
                    </div>
                    ...
                    <div className='flex flex-col gap-y-2'>
                      <p className='text-[#FFFFFF] text-[12px]'>Status</p>
                      {
                        detail?.status === 'in_review' && (
                          <span className='text-[#1877F2] text-[12px] flex items-center gap-x-2 font-bold'> <Icons type='uploaded' /> Uploaded </span>
                        )
                      }     
                      {
                        detail?.status === 'completed' && (
                          <span className='text-green-500 text-[12px] flex items-center gap-x-2 font-bold'> Completed </span>
                        )
                      }    
                      {
                        detail?.status === 'rejected' && (
                          <span className='text-red-500 text-[12px] flex items-center gap-x-2 font-bold'> Rejected </span>
                        )
                      }             
                    </div>   
                </div> 
