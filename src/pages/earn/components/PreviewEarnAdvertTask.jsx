                ...
                <div className='flex-col justify-start items-start gap-1.5 flex'>
                  ...
                  <div className="capitalize text-3xl font-medium font-['Manrope']">
                    {fetchTaskPreview?.task?.task_type === 'advert'
                      ? `Post Advert on your ${fetchTaskPreview?.task?.platform} Page`
                      : (fetchTaskPreview?.task?.goal === 'comment' &&
                          `Comment on ${fetchTaskPreview?.task?.platform} Post`) ||
                        (fetchTaskPreview?.task?.goal === 'follow and like' &&
                          `Follow and Like ${fetchTaskPreview?.task?.platform} Page`) ||
                        (fetchTaskPreview?.task?.goal === 'follow' &&
                          `Follow ${fetchTaskPreview?.task?.platform} Page`) ||
                        (fetchTaskPreview?.task?.goal === 'like' &&
                          `Like ${fetchTaskPreview?.task?.platform} Post`)}
                  </div>
                  <div className='py-1.5 justify-start items-center gap-2 inline-flex'>
                    ...
                    <div className=" text-sm font-bold font-['Manrope']">
                      {fetchTaskPreview?.task?.task_type === 'advert'
                        ? fetchTaskPreview?.task?.platform === 'whatsapp'
                          ? '#60 per Advert post'
                          : ` ₦110 per Advert post`
                        : (fetchTaskPreview?.task?.goal === 'comment' &&
                            ` ₦20 per comment`) ||
                ...
                <div className='grow shrink basis-0 justify-start items-center gap-2.5 flex'>
                  ...
                  {fetchTaskPreview?.task?.task_type === 'advert' && (
                    <div className="grow shrink basis-0 text-blue-600 text-xs font-normal font-['Manrope']">
                      ...
                      {fetchTaskPreview?.task?.platform?.toUpperCase()}
                      {''} advert post after posting. Your MacketIT³ account
                      will be suspended once you DELETE the{' '}
                      {fetchTaskPreview?.task?.platform?.toUpperCase()} Post.
                    </div>
                  )}
                  <Icons type='caution' />
                </div>
                    ...
                    {fetchTaskPreview?.task?.task_type === 'engagement' && (
                      <div className='self-stretch flex flex-col gap-y-2'>
                        ...
                        <span className=" text-xs font-normal font-['Manrope']">
                          ...
                          liking or following the page. You MUST NOT Unfollow
                        ...
                        <span className=" text-xs font-normal font-['Manrope']">
                          ...
                          {fetchTaskPreview?.task?.platform} {''} Username or
                          Name which you used to perform the task
                        </span>
                      </div>
                    )}
                    {fetchTaskPreview?.task?.task_type === 'advert' && (
                      <div className='flex flex-col gap-y-6 py-10 items-center -mt-10 lg:w-full w-11/12'>
                        <div className='text-[12px] font-semibold flex flex-col gap-y-6 px-4'>
                          ...
                          do your task:
                          <p>
                            Step 1: Open {fetchTaskPreview?.task?.platform} on
                            your Mobile App or browser
                          </p>
                          <p>
                            Step 2: Create a post, copy the information below on
                            the description of the advert which you are meant to
                            post on your page.
                          </p>
                          <p>
                            Step 3: Add the image or video provided for the
                            advert. Make sure it's high-quality and visually
                            appealing and post it on your page
                          </p>
                          <p>
                            Step 4: After you have created the new post, then
                            you will comeback to this page and upload the proof
                            of work, which is the link to your profile on{' '}
                            {fetchTaskPreview?.task?.platform} and the link to
                            the {fetchTaskPreview?.task?.platformn} post which
                            you have created
                          </p>
                        </div>
                    ...
                    {fetchTaskPreview?.task?.account_link && (
                      ...
                      >
                        ...
                        <div className='justify-start items-center gap-2 flex'>
                          ...
                          <Link
                            href={fetchTaskPreview?.task?.account_link}
                            className="text-fuchsia-600 text-sm font-medium font-['Manrope']"
                          >
                            Visit Link
                          </Link>
                        </div>
                    ...
                    <div className='self-stretch p-3 bg-rose-100 justify-start items-start gap-[29px] inline-flex'>
                      <div className='grow shrink basis-0 h-[50px] justify-start items-center gap-2.5 flex'>
                        <div className="grow shrink basis-0 text-orange-600 text-xs font-normal font-['Manrope']">
                          ...
                          Your MacketIT³ account will be suspended once you
                    ...
                    {fetchTaskPreview?.account_name && (
                      <div className='flex flex-col gap-y-4'>
                        <p className='text-[12px] font-semibold'>
                          The username on your{' '}
                          {fetchTaskPreview?.task?.platform} account that
                          performed this task
                        </p>
                ...
                {fetchTaskPreview?.task?.task_type === 'advert' && (
                  <div className='w-full flex flex-col gap-y-4'>
                    <div className='text-[12px] flex flex-col gap-y-2 font-normal w-full'>
                      The link to your {fetchTaskPreview?.task?.platform}{' '}
                      profile
                    ...
                    <div className='text-[12px] flex flex-col font-normal gap-y-2 w-full'>
                      The link to the advert post which you created on{' '}
                      {fetchTaskPreview?.task?.platform}
                      <div className='bg-zinc-700 py-4 pl-4 rounded'>
